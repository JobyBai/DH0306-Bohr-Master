...
uint8_t motor_cycle_msg[6] = { 0xAA, 0x1D, 0x01, 0x00, 0xFA, 0x55 }; // 设置电机周期
	...
	if (screen_id == 0x03) // 受迫振荡
		...
		case 0x03: // 速度+
			if (state == 0x01) {
				...
				// }
				motor_cycle += step_table[step_index];
        		if (motor_cycle > 2.0f) motor_cycle = 2.0f;  // 加完再夹紧
		...
		case 0x05: // 速度-
			if (state == 0x01) {
				...
				// }
				motor_cycle -= step_table[step_index];
        		if (motor_cycle < 1.0f) motor_cycle = 1.0f;  // 减完再夹紧
				uint8_t int_part = (uint8_t) motor_cycle;             // 整数秒
				uint16_t ms_part = (uint16_t) ((motor_cycle - int_part) * 1000
						+ 0.5f); // 小数转毫秒，四舍五入
		...
		switch (control_id) {
			case 0x02:
				if (state == 0x01) {
					...
					if (count == 0 || line_flag > count || line_flag < 1) {  // 注意：索引从1开始，所以 line_flag < 1
						line_flag = -1;
						return;
					}
					Record_Delete(5, 1, line_flag);

					for (uint8_t i = line_flag; i < count; i++) {
						forced_record_buf[i] = forced_record_buf[i + 1];
						forced_record_buf[i].index = i;
					}
					memset(&forced_record_buf[count], 0, sizeof(Record_t));
					line_flag = -1;
					index_mode3--;  // 记录数-1，等价于count变为count-1

					Record_Clear(5, 1);
					uint8_t new_count = index_mode3 - 1;
					for (uint8_t i = 1; i <= new_count; i++) {  // 从1开始遍历
						snprintf((char *)output_str, sizeof(output_str),
								"%lu;%.1f;%.4f;%.4f;%.1f;%.1f",
		...
		case 0x04:
			if (state == 0x01) {
				...
				if (connect_type == 0x04) {
					check_wifi_connect_state();
					HAL_Delay(100);
					uint8_t ack_data[8 + 1 + 1 + 1 + count * 6 * 4 + 1];  // 数组大小用count

					memcpy(ack_data, device_id, sizeof(device_id));
					ack_data[8] = 0x51;
					ack_data[9] = count;   // count字段用实际记录数
					ack_data[10] = 0x00;
					uint8_t *ptr = ack_data + 11;
					for (int i = 1; i <= count; i++) {  // 从1开始遍历
						float period_x10 = forced_record_buf[i].period * 10.0f;
						memcpy(ptr, (uint8_t*)&period_x10, sizeof(float));
						ptr += sizeof(float);
						memcpy(ptr, (uint8_t*)&forced_record_buf[i].period, sizeof(float));
						ptr += sizeof(float);
						memcpy(ptr, (uint8_t*)&forced_record_buf[i].drive_period, sizeof(float));
						ptr += sizeof(float);
						memcpy(ptr, (uint8_t*)&forced_record_buf[i].amplitude, sizeof(float));
						ptr += sizeof(float);
						memcpy(ptr, (uint8_t*)&forced_record_buf[i].phase_diff1, sizeof(float));
						ptr += sizeof(float);
						memcpy(ptr, (uint8_t*)&forced_record_buf[i].phase_diff2, sizeof(float));
						ptr += sizeof(float);
					}
					ack_data[sizeof(ack_data) - 1] = 0x13;
					ESP32_ack_data(ack_data, sizeof(ack_data));

				} else if (connect_type == 0x07) {
					ble_transfer_cancel = 1;
					BleUpload_Start(BLE_MODE_FORCED);
				}
			}
			break;
		default:
