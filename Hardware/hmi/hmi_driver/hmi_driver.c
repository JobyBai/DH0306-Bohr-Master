...
float test=0;
void Write_quxianC_float(uint16_t screen_id,uint16_t control_id,uint16_t color,uint16_t *dotx1,uint16_t *doty1,uint16_t dot_cnt,struct LINE_Float_STRUCT *p)
{
	uint16_t i,jiange;
	float X_point,Y_point;
	float X_length1,X_length2,Y_length1;
	uint16_t count;
	uint16_t dotx[201],doty[201];
	uint16_t dot_xandy[202];

	if(dot_cnt>0)
	{
		jiange=dot_cnt/100+1;
		i=0;
		while((i*jiange)<dot_cnt)
		{
			dotx[i]=dotx1[i*jiange];
			doty[i]=doty1[i*jiange]+test;
			i++;
		}

		count=i-1;
    	X_length1=p->QUXIAN_R_DOWN_X-p->QUXIAN_L_UP_X;
		X_length2=p->max_X-p->min_X;
		Y_length1=p->QUXIAN_R_DOWN_Y-p->QUXIAN_L_UP_Y;

		for(i=0;i<=count;i++)
		{

	  	    X_point=X_length1*(dotx[i]-p->min_X)/(X_length2)+p->QUXIAN_L_UP_X;
			Y_point=p->QUXIAN_R_DOWN_Y-(Y_length1*(doty[i]-p->min_Y)/(p->max_Y-p->min_Y));

			if(Y_point<p->QUXIAN_L_UP_Y) Y_point=p->QUXIAN_L_UP_Y;
			if(Y_point>p->QUXIAN_R_DOWN_Y) Y_point=p->QUXIAN_R_DOWN_Y;
			if(X_point<p->QUXIAN_L_UP_X) X_point=p->QUXIAN_L_UP_X;
			if(X_point>p->QUXIAN_R_DOWN_X) X_point=p->QUXIAN_R_DOWN_X;

			 dot_xandy[2*i]= X_point;
			 dot_xandy[2*i+1]= Y_point;
		}
		// dot_xandy[2*count]= dot_xandy[0];
		//	dot_xandy[2*count+1]= dot_xandy[1];
...
*  \param
*/
void GUI_Clean(uint16_t Enable)
{
	BEGIN_CMD();
	TX_8(0x06);
	TX_8(Enable);
...
void Record_Add123(uint16_t screen_id,uint16_t control_id,float value)
{
    BEGIN_CMD();
    TX_8(0xB1);
    TX_8(0x52);
...
void Record_Add2(uint16_t screen_id,uint16_t control_id,uint8_t *record,uint8_t *record1,uint8_t *record2,uint8_t *record3,uint8_t *record4)
{
    BEGIN_CMD();
    TX_8(0xB1);
    TX_8(0x52);
...
void Line1_init(struct LINE_STRUCT *p)
{
p->QUXIAN_L_UP_X=30; //曲线区域左上角X坐标
p->QUXIAN_L_UP_Y=74; //曲线区域左上角Y坐标
p->QUXIAN_R_DOWN_X=555; //曲线区域右下角X坐标
p->QUXIAN_R_DOWN_Y=456; //曲线区域右下角Y坐标
...
void Line2_init(struct LINE_STRUCT *p)
{
p->QUXIAN_L_UP_X=30; //曲线区域左上角X坐标
p->QUXIAN_L_UP_Y=74; //曲线区域左上角Y坐标
p->QUXIAN_R_DOWN_X=555; //曲线区域右下角X坐标
...
p->max_X=30000;//X测量最大值
p->max_Y=20000;//Y测量最大值
p->min_X=-4000;//X测量最小值
p->min_Y=-10000;//Y测量最小值
}


void Write_quxian(uint16_t screen_id,uint16_t control_id,int16_t *dotx1,int16_t *doty1,uint16_t dot_cnt,struct LINE_STRUCT *p) //注意dot_cnt为实际个数，而数组从0开始
{   uint16_t i,jiange;
	  int16_t X_point,Y_point;
	  int16_t X_length1,X_length2,Y_length1,Y_length2;
		uint16_t count;
	  int16_t  dotx[1024],doty[1024];
	  int16_t  dot_xandy[900];//数组合并
	...
	if(dot_cnt>0)
	{
		...
		{
			dotx[i]=dotx1[i*jiange];
			doty[i]=doty1[i*jiange];//曲线无法显示负数部分，曲线向上平移10000实现显示负数部分
			i++;
		}
   	...
   	X_length1=p->QUXIAN_R_DOWN_X-p->QUXIAN_L_UP_X;
		...
		for(i=0;i<=count;i++)
		{
	  	X_point=X_length1*(dotx[i]-p->min_X)/(X_length2)+p->QUXIAN_L_UP_X;
			...
			cs2[3]=X_point;
			cs2[4]=dotx[i];
			Y_point=p->QUXIAN_R_DOWN_Y-(Y_length1*(doty[i]-p->min_Y)/(Y_length2));

			if(Y_point<p->QUXIAN_L_UP_Y)
			   Y_point=p->QUXIAN_L_UP_Y;
    ...
    TX_16(control_id);
	  ...
	  TX_8(0x01);
	  TX_8(0xFF);
    TX_16N(dot_xandy,(count+1)*2);
    END_CMD();

	}
}

void GUI_FcolorConDots2(uint16_t *dotx1,uint16_t *doty1,uint16_t dot_cnt,struct LINE_STRUCT *p) //注意dot_cnt为实际个数，而数组从0开始
{   uint16_t i,jiange;
	  uint16_t X_point,Y_point;
	  uint16_t X_length1,X_length2,Y_length1,Y_length2;
		uint16_t count;
	  uint16_t dotx[1024],doty[1024];
	  uint16_t dot_xandy[900];//数组合并
	//SetBcolor(65535);
	GUI_RectangleFill(17,64,549,457);
	if(dot_cnt>0)
	{
		jiange=dot_cnt/60+1;
		i=0;
   	...
   	X_length1=p->QUXIAN_R_DOWN_X-p->QUXIAN_L_UP_X;
		...
		for(i=0;i<=count;i++)
		{
	  	X_point=X_length1*(dotx[i]-dotx[0])/(X_length2)+p->QUXIAN_L_UP_X;
			Y_point=p->QUXIAN_R_DOWN_Y-(Y_length1*(doty[i]-p->min_Y)/(Y_length2));
...
void Write_quxian_ENABLE(uint16_t X,uint16_t Y,uint16_t W,uint16_t H)
{
...
void Write_quxian1(uint16_t screen_id,uint16_t control_id,uint16_t X0,uint16_t X1,uint16_t Y0,uint16_t Y1)
{
    BEGIN_CMD();
    TX_8(0xB1);
    TX_8(0x10);
    ...
    TX_16(X0);
	  ...
	  TX_16(Y1);
    END_CMD();
}//EE B1 10 00 00 00 01 02 01 FF 00 32 00 32 00 C8 00 C8 00 64 00 64 FF FC FF FF

void Clear_quxian(uint16_t screen_id,uint16_t control_id)
{
    BEGIN_CMD();
    TX_8(0xB1);
    TX_8(0x10);
...
void tanchuang(uint16_t screen_id)
{
    BEGIN_CMD();
    TX_8(0xB1);
    TX_8(0x0A);
...
void biaoge_bottom(uint16_t screen_id,uint16_t control_id,uint16_t hangshu)
{
    BEGIN_CMD();
    TX_8(0xB1);
    TX_8(0x54);
 ...
 * @param row 行号
 */
void Record_Delete(uint16_t screen_id, uint16_t control_id, uint16_t row)
{
    BEGIN_CMD();
    TX_8(0xB1);
    TX_8(0x58);
